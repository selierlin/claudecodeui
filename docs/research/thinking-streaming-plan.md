...
> **状态**：待审阅（计划阶段产出，尚未开始实现；**§8 前置实测已完成**，结论已回填 §2.2 / §5-0 / §6-0 / §7-Q5）
> **日期**：2026-09-11
...
**一个被低估的细节**：思考面板默认是**收起**的（`Reasoning defaultOpen={isExporting}` → false）。也就是说——即使做了路线 A，若不做自动展开，用户看到的仍只是一个折叠标题，静默观感**几乎没有改善**。这使「A 必须配套自动展开」成为硬约束；反过来说，B 用很小成本覆盖了同样的「看得见它在动」这一核心诉求。
...
1. B 的成本与 A 差一个量级，而**核心痛点是「静默」而非「看不到思考内容」**——思考内容默认收起，A 的大部分价值要靠额外改动（自动展开）才能兑现。
2. 上一轮文本流式刚改完、手工验收尚未做；此时再叠一个横跨前后端的通道模型，会把验证面成倍放大。
3. A 与 B **不互斥**：B 的指示器在 A 落地后仍可作为「思考内容被 redacted / display=omitted」时的降级显示（此时只有 `estimated_tokens`，没有 `thinking` 文本）。

若审阅者认为「思考内容可见」本身就是产品价值（对齐 Claude.ai 的展开体验），则选 A，但必须同时落自动展开。
...
### 4.1 折叠面板让 A 的收益落空（高，若选 A）

见 §2.4。A 必须配套「流式期间自动展开」，否则等于没做。这是选 A 时最容易漏掉的一步。
...
| 1 | 长思考任务（1 万字小说这类） | **自首个思考 delta 起** ≤ 2s 出现可见反馈（§8.4 口径：不含不可控的 TTFT ≈5.4s），不再有 19s 级静默 |
...
| # | 问题 | 我的倾向 |
|---|---|---|
| Q1 | 路线 A（思考内容流式）还是 B（进度指示）？ | ✅ **已定案（2026-09-11）：走路线 B**。作者与审阅者一致；A 保留为可选后续（若「思考内容可见」成为产品目标再做，且必须配套自动展开） |
...
Q1（先 B）、Q4（指示器不受 `showThinking` 控制）、Q5（纳入合并，最坏值按 96% 计）均与批注一致，无异议。**Q3（P1）接受批注的条件**：P1 的「改动面最小」确实被低估了——合成流式思考行的 prune 判据 / id 方案是 P1 落地的必要组成（§4.7.2），已写回 §2.1 与 §3，不再把 P1 描述为「只需扩展一个维度」。
